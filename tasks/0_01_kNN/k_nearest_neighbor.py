...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        mod_test = np.sum(X**2, axis=1, keepdims=True)
        mod_train = np.sum(self.X_train**2, axis=1, keepdims=True)
        dists = np.sqrt(mod_test + mod_train.T - 2 * X.dot(self.X_train.T))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            neighbors.append(np.argsort(dists[i])[:k])
            closest_y.append(self.y_train[neighbors[-1]])
