...
class DecisionTree(BaseEstimator):
    ...
    def make_split(self, feature_index, threshold, X_subset, y_subset):
        ...
        not_ind = X_subset[:,feature_index] >= threshold
        
        X_left = X_subset[ind]
        y_left = y_subset[ind]
        X_right = X_subset[not_ind]
        y_right = y_subset[not_ind]
    ...
    def choose_best_split(self, X_subset, y_subset):
        ...
        min_loss = np.inf
        threshold, feature_index = None, None
        loss_func = self.all_criterions[self.criterion_name][0]

        for j in range(X_subset.shape[1]):
            for t in X_subset[:, j]:  # feature as threshold
                y_left,  y_right = self.make_split_only_y(j, t, X_subset, y_subset)
                loss = (y_left.shape[0])*loss_func(y_left) + (y_right.shape[0])*loss_func(y_right)
                if loss < min_loss:
                    min_loss = loss
                    feature_index = j
                    threshold  = t
        ...
        for i in range(X.shape[0]):
            ...
            while type(curr_node) is Node:
                if X[i][curr_node.feature_index] < curr_node.value:
                    ...
                    curr_node = curr_node.right_child
            y_predicted_probs[i] = np.max(curr_node)
